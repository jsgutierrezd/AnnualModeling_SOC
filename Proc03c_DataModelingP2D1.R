...
corMat <- cor(as.matrix(dataP1D2[,-c(6:23)])) %>% na.omit %>% as.data.frame
corMat <- dataP1D2[,-c(6:23)] %>% na.omit %>% cor %>% data.frame

write_csv(corMat,"CorMatRegMat_P1D2.csv")
...
set.seed(1536)
inTrain <- createDataPartition(y = dataP1D2$SOC_1986t, p = .70, list = FALSE)

train_data <- dataP1D2[ inTrain,]
test_data <- dataP1D2[-inTrain,]
...
saveRDS(model_qrf,"ModelP1D2qrf_220722.rds")
