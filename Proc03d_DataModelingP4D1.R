...
{
  start <- Sys.time()
  (bor <- Boruta(x = train_data[,c(2:ncol(train_data))],
                 y = train_data[,1], 
                 #data = train_data, 
                 doTrace = 0, 
                 ntree = 500,
...
unc <- clusterR(covP3[[names(bor$finalDecision[bor$finalDecision %in% c("Confirmed")])]], predict, 
                args=list(model=model_qrf,what=sd))
unc <- exp(unc)

mean <- clusterR(covP3[[names(bor$finalDecision[bor$finalDecision %in% c("Confirmed")])]], predict,
...
plot(mean)
plot(unc)
writeRaster(mean,"Outputs/Layers/ModelP3D1QrfMean_260722.tif")
writeRaster(unc,"Outputs/Layers/ModelP3D1QrfUnc_260722.tif")
